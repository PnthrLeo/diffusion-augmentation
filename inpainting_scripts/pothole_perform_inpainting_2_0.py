...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Perform inpainting on pothole images')
    parser.add_argument('--dataset_path', type=str, required=True, help='Path to the pothole dataset')
    parser.add_argument('--inpainting_output_path', type=str, required=True, help='Path to the inpainting output')
    parser.add_argument('--inp_img_per_orig_img', type=int, required=True, help='Number of inpainted images per image required', default=6)
    args = parser.parse_args()
    
    pipeline_path = 'batch_inpainting_SDXL_refiner_controlnet-v2.0-p-ready-no-depth-pothole-api.json'
    ...
    for part_idx in range(len(images_paths)):
        ...
        # set inpainting images saver path v2
        ...
        positive_prompts = ['a photo of a road, (((ultrarealistic))), hdr, 8k']
