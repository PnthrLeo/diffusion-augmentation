...
if __name__ == '__main__':
    ...
    pipeline_path = 'batch_inpainting_SDXL_refiner_controlnet-v1.2-p-ready-no-depth-mvtec-api.json'
    ...
    for part_idx in range(len(images_paths)):
        ...
        positive_prompts = ['a photo of a bottle bottom, (((ultrarealistic))), hdr, 8k']
        ...
        while inpainted_images_counter < number_of_inpainted_images_per_image_required:
            ...
            for idx, prompt in enumerate(positive_prompts):
                ...
                for jdx in range(len(list(Path(images_paths[part_idx]).glob('*.png')))):
                    ...
                    pipeline['121']['inputs']['text'] = str(inpainted_images_counter)
                    pipeline['122']['inputs']['text'] = str(idx)
